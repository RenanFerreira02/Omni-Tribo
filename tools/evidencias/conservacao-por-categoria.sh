...
# AJUDA cunha do nada -> a soma deve crescer  (Δ > 0)
#
# Em ambos os casos, a RECONCILIAÇÃO (ledger × projeção) deve continuar respondendo integro=true —
# é exatamente esse o ponto: são invariantes diferentes.
set -uo pipefail
...
# ─────────────────────────────────────────────────────────────────────────────
echo
echo "############ CICLO 1 — AJUDA (esperado: CUNHAGEM, Δ > 0) ############"
A1=$(medir | cut -d'|' -f3)
PREVIA=$(api POST /api/v1/missoes/previa-recompensa "$ALICE" "$(corpo AJUDA 'F13 AJUDA')")
...
M1=$(api POST /api/v1/missoes "$ALICE" "$(corpo AJUDA 'F13 AJUDA')")
...
echo "publicar: $(api POST "/api/v1/missoes/$M1_ID/publicar" "$ALICE" | jq -r '.status')"
echo "aceitar:  $(api POST "/api/v1/missoes/$M1_ID/aceitar"  "$BOB"   | jq -r '.status')"
echo "iniciar:  $(api POST "/api/v1/missoes/$M1_ID/iniciar"  "$BOB"   | jq -r '.status')"
CHK=$(api POST "/api/v1/missoes/$M1_ID/checkin" "$BOB" \
  "{\"lat\":$ORIGEM_LAT,\"lon\":$ORIGEM_LON,\"acuraciaM\":8.0,\"mocked\":false}")
echo "checkin:  $(echo "$CHK" | jq -r '.status // .type')"
echo "confirmar:$(api POST "/api/v1/missoes/$M1_ID/confirmar" "$ALICE" | jq -r '.status')"
...
TRIBO_ID=$("${PSQL[@]}" "SELECT tribo_id FROM usuario WHERE email='bob@omnitribo.dev';" | tr -d ' ')
...
echo
echo "############ RESUMO ############"
echo "AJUDA  Δ=$((A2-A1))  recompensa=$TOK_AJUDA"
